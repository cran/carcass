...
estimateN <- function(count, p=NA, p.lower=NA, p.upper=NA, f=NA, f.lower=NA, f.upper=NA, 
                        s=NA, s.lower=NA, s.upper=NA, pform="korner", d=1, n=NA, maxn=500, nsim=1000, plot=TRUE, postdist=FALSE){
...
if(pform=="korner") pr <- pkorner(s=sr, f=fr, d=d, n=n)
if(pform=="huso") pr <- phuso(s=sr, f=fr, d=d)
if(pform=="erickson") pr <- perickson(t_quer=-1/log(sr), f=fr, d=d)  # 

postNtemp <- posterior.N(nf=count, p=pr, maxN=maxn,  plot=FALSE, dist=TRUE)
...
estimate <- min(c(0:maxn)[!indexMedian])  
...
if(!postdist) {
result <- c(estimate, lower, upper)
names(result) <- c("estimate", "lower", "upper")
}
if(postdist) {
result <- list(estimate=estimate, lower=lower, upper=upper, postdist=Npostdist.sc)
}
